...
const JAR_COLORS = ["#60a5fa", "#34d399", "#f59e0b", "#f472b6", "#a78bfa"];
...
export default function JarsCard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/jars/allocations`, {
        credentials: "include",
      }).then((r) => r.json()),
      ...
      .then(([jarsData, transactions]) => {
        setJars(jarsData || []);

        const usageMap: Record<number, number> = {};

        (transactions || []).forEach((t: Transaction) => {
          if (t.type?.toLowerCase() === "expense" && t.jar_id) {
            usageMap[t.jar_id] =
              (usageMap[t.jar_id] || 0) + Number(t.amount || 0);
          }
        });
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {fmt(used)} / {fmt(jar.allocated_amount)}
                  </span>
                </div>
