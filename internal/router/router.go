...
func New(tmpl *template.Template, pool *pgxpool.Pool) http.Handler {
	...
	demoH := handlers.NewDemoHandler()

	// In demo mode: no login required, skip auth middleware entirely
	if db.IsDemo() {
		r.Get("/", pageH.Index)
		r.Post("/demo/reset", demoH.Reset)
		registerAppRoutes(r, accountH, txH, optionsH, jarH, tmplH)
		// health
		r.Head("/", func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusOK) })
		return r
	}

	// Prod: login + auth middleware
...
// registerAppRoutes mounts all the data routes — shared between demo and prod.
func registerAppRoutes(
	r chi.Router,
	accountH *handlers.AccountHandler,
	txH *handlers.TransactionHandler,
	optionsH *handlers.OptionsHandler,
	jarH *handlers.JarHandler,
	tmplH *handlers.TxTemplateHandler,
) {
	// accounts
