...
// EnsureSchema creates all tables if they don't exist and adds any missing
// columns. Safe to call on every startup — fully idempotent.
func EnsureSchema(ctx context.Context) {
	steps := []struct {
		...
		{
			"accounts.archived_at column",
			`ALTER TABLE accounts ADD COLUMN IF NOT EXISTS archived_at TIMESTAMP DEFAULT NULL`,
		},
	...
	for _, s := range steps {
		if _, err := Pool.Exec(ctx, s.sql); err != nil {
			log.Fatalf("migration %q failed: %v", s.name, err)
		}
		log.Printf("migration ok: %s", s.name)
	}
}
